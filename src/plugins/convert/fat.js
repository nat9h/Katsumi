...
export default {
	name: "fat",
	description: "Add fat / thick bass effect to audio/video.",
	command: ["fat", "fatbass", "thicc"],
	permissions: "all",
	hidden: false,
	...
	execute: async (m) => {
		...
		try {
			...
			execSync(
				`ffmpeg -y -i "${inputPath}" -af "bass=g=15:f=60:w=0.8,lowpass=f=3000,volume=1.5" -vn "${outputPath}"`,
				{ stdio: "ignore" }
			);
