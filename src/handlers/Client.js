...
export class Client extends EventEmitter {
    ...
    /**
     * IDs of messages this bot generated, so we can recognize our own echoes
     * without tagging the wire-level message ID. Short TTL: echoes arrive
     * within seconds, so a generated ID never needs to live long.
     */
    #sentMsgIds = new MemCache({ ttl: 10 * 60_000, max: 5000 });
    ...
    /**
     * Generate a clean WhatsApp-style message ID (3EB0…, identical to the
     * official web client) and remember it so {@link wasSentByMe} can
     * recognize our own echoes later.
     */
    generateMsgId() {
        const id = generateMessageIDV2(this.sock?.user?.id);
        this.#sentMsgIds.set(id, true);
        return id;
    }

    /**
     * Whether a message ID was generated by this bot.
     * @param {string|undefined} id
     * @returns {boolean}
     */
    wasSentByMe(id) {
        return id ? this.#sentMsgIds.peek(id) === true : false;
    }
